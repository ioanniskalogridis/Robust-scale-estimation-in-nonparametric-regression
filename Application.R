...
# kc1 <- importAURN(site = "SCN2", year = 2024)  
...
plot(x,y, cex.axis = 2.5, cex.lab = 2.5, cex = 2.5, pch = 19, col = "gray",
     ylab = "NOx (PPB)", xlab = "Temperature (\u00B0C)", ylim = c(0, 280)) ; grid()
...
plot(x[abs(fit.ls1$st.resids)<=cut.off],y[abs(fit.ls1$st.resids)<=cut.off], cex.axis = 2.5, cex.lab = 2.5, cex = 2.5, pch = 19, col = "gray",
     ylab = "NOx (PPB)", xlab = "Temperature (\u00B0C)", ylim = c(0, 280)) ; grid()
